...
class Config:
    def __init__(self, config_file_path):
        ...
        self.QD_classes = [(c, int(n)) for c,n in config['classes'].items()]
        self.QD_class_index = {c: n for c, n in self.QD_classes}
        self.QD_class_name = {n: c for c, n in config['classes'].items()}
        self.QD_colors = config['display']['colors']
        self.color_table = [self.QD_colors[self.QD_class_name[i]] if i in self.QD_class_name else 'black'
                            for i in range(max(list(self.QD_class_name.keys())))]
        self.QD_cmap = matplotlib.colors.ListedColormap(self.color_table)
